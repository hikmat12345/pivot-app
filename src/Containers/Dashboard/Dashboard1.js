...
class Dashboard extends Component {
  ...
  groupingLevel = async pivotdata => {
    ...
    pivotdata.map((value, index) => {
      //filterxero rows
      var checkzero = false;
      if (value.Columns && value.Columns.length > 0 && value.Type !== "Total") {
    ...
    this.state.orderColumns.map((vals, ind) => {
      if (vals.ColumnName == "Level") {
        levelguid = vals.guid;
      }
      if (vals.ColumnName == "Description") {
        descriptionguid = vals.guid;
      }
      if (vals.ColumnName == "Account") {
        accountguid = vals.guid;
      }
      if (vals.ColumnName == "Groups") {
        groupguid = vals.guid;
      }
    });
    ...
    totalofall.map((value, index) => {
      if (
        ...
        if (value.obj.TextValue == "0" || value.obj.TextValue == NaN) {
          value.obj.TextValue = " ";
        }
  ...
  handleChangeCheckbox = (checkBoxName, isChecked) => {
  ...
  insertRowHandler = async () => {
    ...
    // var samplefordb = this.state.originalData.find(e => e.Type === "Detail");
    ...
    check_row[0].Columns.map(s => {
      var column = this.state.orderColumns.find(column => column.guid === s.obj.ColumnGuid);
      columnsfordb.push({        
        uniqueCellId: uuidv1(),
        ColumnGuid: s.obj.ColumnGuid,
        AmountValue: "0",
        TextValue: column.ColumnName === "Level" ? s.obj.TextValue : null,
        // TextValue: null
      });
    });
    ...
    if (check_row.length == 1) {
        ...
        this.state.required_messages.map(e =>
          e.ID == 17
            ? this.setState({
                ...
                message_heading: "Can't Paste Row",
                openMessageModal: true
              })
            : ""
        );
      ...
      if (having_child_node.length === 0) {
        var rowtoadd = {
          ...
          Position: this.state.pivotGroupTag[checkrowindex - 1].Position + "1",
  ...
  pasteRow = async () => {
    ...
    var pasteParent = "";
    var pasteChild = "";
    this.state.pivotGroupTag.filter((e, index) => {
      // push checkrow
      if (e.checkbox == true) {
        check_row.push(e);
      }
    ...
    if (check_row.length > 0) {
      pasterow = check_row[0];
      var hindex = this.state.pivotGroupTag.find(
        u => u.guid == pasterow.ParentGuid
      );
      var childernarray = this.state.pivotGroupTag.filter(
        u => u.ParentGuid == hindex.guid && u.Type == "Header"
      );

      if (pasterow.Type == "Header") {
        this.setState({
          message_desc:
          "Can’t paste on header rows",
          message_heading: "Paste",
          openMessageModal: true
        });

        pasterow.checkbox = false;
        await this.setState({ pivotGroupTag: this.state.pivotGroupTag });
      } else if (childernarray.length == 0 && pasterow.Type != "Header") {
        if (cutRows.length > 0) {
          var fire;
          var jj = false;
          var type = "";
          cutRows.map((value, index) => {
            cut = cutRows[index];
            fire = false;
            var x = this.state.pivotGroupTag;

            cutParent = this.state.pivotGroupTag.find(
              value => value.guid == cut.ParentGuid
            );
            if (cut.Type == "Header" || cut.Type == "SubHeader") {
              var hindex = this.state.pivotGroupTag.find(
                u => u.guid == pasterow.ParentGuid
              );
              var childernarray = this.state.pivotGroupTag.filter(
              ...
              if (childernarray.length == 0) {
                var gui = [];
                var aaray = [];
                // get chlidern of cut header
                this.state.pivotGroupTag.map(value => {
                  if (value.ParentGuid == cut.guid) {
                    aaray.push(value);
                  }
                });

                if (cut.ParentGuid == "root") {
                  toast.error("Can't cut and paste root element");
                } else {
                  if (
                    pasterow.Type == "Header" ||
                    pasterow.Type == "SubHeader"
                  ) {
                    cut.ParentGuid = pasterow.guid;
                    var orirow = this.state.originalData.find(
                      o => o.guid == cut.guid
                    );
                    orirow.ParentGuid = pasterow.guid;
                    cut.Position = pasterow.Position + "1";

                    orirow.Position = pasterow.Position + "1";

                    pasteParent = pasterow;
                    cut.cutrow = false;
                    delete cutRows[index];

                    aaray.map((value, index) => {
                      value.Position = cut.Position + index + "";
                      this.state.originalData.find(
                        oi => oi.guid == value.guid
                      ).Position = cut.Position + index + "";
                      if (value.Type == "Header" || value.Type == "SubHeader") {
                        var obj = {
                          guid: value.guid,
                    ...
                    while (gui.length >= 1) {
                      var xx = [];
                      var guis = gui[0];
                      this.state.pivotGroupTag.map((u, index) => {
                        if (u.ParentGuid == guis.guid) {
                          u.Position = guis.Position + index;
                          this.state.originalData.find(
                            iu => iu.guid == u.guid
                          ).Position = guis.Position + index;
                          if (u.Type == "Header" || u.Type == "SubHeader") {
                            var obj = {
                              guid: u.guid,
                              Position: guis.Position + index
                            };
                      ...
                      gui = gui.filter(u => u.guid != guis.guid);
                    }
                  } else if (
                    pasterow.Type != "Header" &&
                    pasterow.Type != "Total"
                    ...
                    var pos = "";
                    var prepaste = "";
                    var pre = this.state.pivotGroupTag.findIndex(
                      u => u.guid == pasterow.guid
                    );
                    prepaste = this.state.pivotGroupTag[pre - 1];

                    cut.ParentGuid = pasterow.ParentGuid;
                    cut.Position = prepaste.Position + "1";
                    cut.cutrow = false;
                    var x = this.state.originalData.find(
                      u => u.guid == cut.guid
                    );
                    x.ParentGuid = pasterow.ParentGuid;

                    x.Position = prepaste.Position + "1";

                    pasterow.Position = cut.Position + "1";
                    ...
                    aaray.map((value, index) => {
                      ...
                      pasterow.Position = value.Position + index + "1";
                      if (value.Type == "Header" || value.Type == "SubHeader") {
                        var obj = {
                          guid: value.guid,
                          Position: value.Position
                    ...
                    var ind = this.state.pivotGroupTag.find(
                      u => u.guid == pasterow.ParentGuid
                    );

                    if (ind.ParentGuid !== "root") {
                      cut.ParentGuid = ind.guid;
                      cut.Position = prepaste.Position + "1";
                      cut.cutrow = false;
                      var x = this.state.originalData.find(
                        u => u.guid == cut.guid
                      );
                      x.ParentGuid = ind.guid;
                      x.Position = prepaste.Position + "1";

                      this.state.originalData.find(
                        o => o.guid == pasterow.guid
                      ).Position = cut.Position + "1";
                      pasterow.Position = cut.Position + "1";
                      pasteParent = ind;
                      delete cutRows[index];
                      aaray.map((value, index) => {
                      ...
                      pasterow.cutrow = false;
                    } else if (ind.ParentGuid == "root") {
                      fire = true;
                      this.state.required_messages.map(e =>
                        e.ID == 17
                          ? this.setState({
                ...
                toast.error("Can't paste over here");
              }
            } else if (cut.Type != "Header" && cut.Type != "Total") {
              this.state.pivotGroupTag.map(async e => {
                if (e.guid == pasterow.guid) {
                  var hindex = this.state.pivotGroupTag.find(
                    u => u.guid == e.ParentGuid
                  );
                  var childernarray = this.state.pivotGroupTag.filter(
                    u => u.ParentGuid == hindex.guid && u.Type == "Header"
                  );
                  if (childernarray.length == 0) {
                    if (e.Type == "Header" || e.Type == "SubHeader") {
                      toast.error("can not paste here");
                    }
                    ...
                    if (
                      ...
                      cut.Position = prepaste.Position + "1";
                      cut.cutrow = false;
                      e.Position = cut.Position + "11";
                      var x = this.state.originalData.find(
                        u => u.guid == cut.guid
                      );
                      x.ParentGuid = pasterow.ParentGuid;
                      x.Position = prepaste.Position + "1";
                      // cutColumnOrigionalData start
                      // var cutColumnOrigionalData = x.Columns.find(column => this.state.orderColumns.find(orderColumn => column.ColumnGuid === orderColumn.guid && orderColumn.ColumnName === "Level"))
                      // cutColumnOrigionalData end

                      this.state.originalData.find(
                        u => u.guid == e.guid
                      ).Position = cut.Position + "11";
                      pasteParent = "";
                      pasteChild = e;
                      
                      // pasteColumnOrigionalData start
                      // var pasteColumnOrigionalData = pasteChild.Columns.find(column => this.state.orderColumns.find(orderColumn => column.obj.ColumnGuid === orderColumn.guid && orderColumn.ColumnName === "Level"))
                      // cutColumnOrigionalData.TextValue = pasteColumnOrigionalData.obj.TextValue;
                      // pasteColumnOrigionalData end

                      var totalchild = this.state.pivotGroupTag.filter(
                        u => u.ParentGuid == pasterow.ParentGuid
                      );
                      var ind = totalchild.findIndex(
                        e => e.guid == pasterow.guid
                      );
                    ...
                    if (e.Type == "Total") {
                      var indi = this.state.pivotGroupTag.find(
                        value => value.guid == pasterow.ParentGuid
                      );
                      if (indi.ParentGuid != "root") {
                        var prepaste = "";
                        var pre = this.state.pivotGroupTag.findIndex(
                          u => u.guid == pasterow.guid
                        );
                        ...
                        cut.ParentGuid = e.ParentGuid;
                        cut.Position = prepaste.Position + "1";
                        e.Position = cut.Position + "1";
                        cut.cutrow = false;
                        delete cutRows[index];
                        pasterow.cutrow = false;
                        var x = this.state.originalData.find(
                        ...
                        x.ParentGuid = e.ParentGuid;
                        x.Position = prepaste.Position + "1";

                        this.state.originalData.find(
                          u => u.guid == e.guid
                        ).Position = cut.Position + "1";
                      } else if (indi.ParentGuid == "root") {
                        fire = true;
                        this.state.required_messages.map(e =>
                          e.ID == 17
                            ? this.setState({
                    ...
                    toast.error("Can not Paste please select valid node");
                  }
                }
              });
          ...
          if (pasteParent == "") {
            pasteParent = this.state.finaldata.find(
              u => u.guid == pasteChild.ParentGuid
            );
          }
          var tes = this.state.finaldata;
          if (
            pasterow.Type === "Header" &&
            pasterow.clicked == false &&
            fire == false
          ) {
            console.log("in iffff");
            tes = this.state.finaldata.filter(val => {
              return this.state.cutfinal.indexOf(val.guid) === -1;
            });
            await this.setState({
            ...
            await this.setState({
              ...
              finaldata: tes.sort(function(a, b) {
                var nameA = a.Position;
                var nameB = b.Position;
                if (nameA < nameB) {
                  return -1;
          ...
          if (fire == false) {
            ...
            await this.calculateData(pasteParent);

            await this.calculateData(cutParent);
          }
        } else if (cutRows.length != 1) {
          this.state.required_messages.map(e =>
            e.ID == 18
              ? this.setState({
                  message_desc: e.Desc,
      ...
      } else if (childernarray.length > 0) {
        toast.error("Can't paste ");
      }
    ...
    await this.setState({ originalData: this.state.originalData });

    //    await API.post("pivot", "/copypivotdata", {
    //      body: {
    //        pivotdata: this.state.originalData
    //      }
    //    }).then(data => {
    //      toast.success("data added")
    //    }).catch(err => {
  ...
  clickgroups = async () => {
    var fla = false;
    var selectedrows = [];
    var groupguid = this.state.orderColumns.find(u => u.ColumnName == "Groups");
    this.state.finaldata.filter((e, index) => {
      if (e.checkbox == true) {
        if (e.Type == "Blank") {
          fla = true;
  ...
  groupcheckedvalue = async obj => {
    await this.setState({ groupValue: obj, isLoading: true });
    var x = this.state.orderColumns.find(u => u.ColumnName == "Groups");
    ...
    pi.map((u, i) => {
      p[u].Columns.map(i => {
        if (i.obj.ColumnGuid == groupguid) {
          if (i.obj.TextValue) {
            i.obj.TextValue = this.state.groupValue;
          } else {
            i.obj.AmountValue = this.state.groupValue;
          }
        }
      });
      var hh = fi[i];
      ...
      var orgdata = od[i];
      originaldata[orgdata].Columns.map(i => {
        if (i.ColumnGuid == groupguid) {
          if (i.TextValue) {
            i.TextValue = this.state.groupValue;
          } else {
            i.AmountValue = this.state.groupValue;
          }
        }
      });
    });
  ...
  insertHeaderTotal = async () => {
    ...
    if (selectedrows.length == 2) {
      if (
        ...
        if (
          selectedrows[1].Type == "Total" &&
          selectedrows[1].ParentGuid == selectedrows[0].guid
        ) {
          var childern = this.state.pivotGroupTag.filter(
            u => u.ParentGuid == selectedrows[0].guid
          );
          var detail_row = childern.filter(u => u.Type == "Detail");
          if (detail_row.length == 0) {
            ...
            var guid = uuidv1();
            var tempcol = [];
            selectedrows[0].Columns.map(ioi => {
              var obj1 = {
                ColumnGuid: ioi.obj.ColumnGuid,
                AmountValue: ioi.obj.AmountValue,
                TextValue: ioi.obj.TextValue,
                uniqueCellId: uuidv1()
              };
            ...
            var headObj = {
              ...
              Columns: tempcol,
              Position: selectedrows[0].Position - 1,
              TenantGuid: selectedrows[0].TenantGuid,
              TotalGuid: selectedrows[0].TotalGuid,
              Type: "Header",
              guid: guid,
              checkbox: false,
              checkzero: false,
            ...
            let clo = JSON.parse(
              JSON.stringify(
                this.state.originalData.find(p => p.Type == "Header").Columns
              )
            );
            console.log("original---------", clo);

            let cloorg = [];
            clo.map(io => {
              let found = "";

              found = headObjorg.Columns.find(
                op => op.obj.ColumnGuid == io.ColumnGuid
              );
              if (found !== "" && found !== undefined) {
                obj = {
                  ColumnGuid: io.ColumnGuid,
                  AmountValue:
                    found.obj.AmountValue == "" ? "0" : found.obj.AmountValue,
                  TextValue:
                    found.obj.TextValue == "" ? null : found.obj.TextValue
                };
              } else {
                obj = {
                  ColumnGuid: io.ColumnGuid,
            ...
            headObjorg.Columns = cloorg;

            console.log(cloorg, "header======");
            var tempcol1 = [];
            selectedrows[1].Columns.map(ioi => {
              var obj12 = {
                ColumnGuid: ioi.obj.ColumnGuid,
                AmountValue: ioi.obj.AmountValue,
                TextValue: ioi.obj.TextValue,
            ...
            var TotalObj = {
              ...
              ParentGuid: guid,
              Columns: tempcol1,
              Position: selectedrows[1].Position + 1,
              TenantGuid: selectedrows[1].TenantGuid,
              TotalGuid: selectedrows[1].TotalGuid,
              Type: "Total",
              guid: uuidv1(),
              checkbox: false,
              checkzero: false,
            ...
            let totalobjs = JSON.parse(JSON.stringify(TotalObj));
            let clos = JSON.parse(
              JSON.stringify(
                this.state.originalData.find(p => p.Type == "Total").Columns
              )
            );
            console.log(clos, "original--------");

            let clrg = [];
            clos.map(io => {
              let found = "";

              found = totalobjs.Columns.find(
                op => op.obj.ColumnGuid == io.ColumnGuid
              );
              if (found !== "" && found !== undefined) {
                var obj = {
            ...
            totalobjs.Columns = clrg;

            console.log(clrg, "<=====clrg");

            selectedrows[0].ParentGuid = guid;

            var xy = this.state.originalData;
            var changerow = xy.find(q => q.guid == selectedrows[0].guid);
            ...
            x.push(headObj);
            x.push(TotalObj);
            var y = this.state.pivotGroupTag;
            y.push(headObj);
            y.push(TotalObj);
            await this.setState({ originalData: xy });

            await this.setState({
              finaldata: x.sort(function(a, b) {
                var nameA = a.Position;
                var nameB = b.Position;
                if (nameA < nameB) {
                  return -1;
              ...
              pivotGroupTag: y.sort(function(a, b) {
                var nameA = a.Position;
                var nameB = b.Position;
                if (nameA < nameB) {
                  return -1;
            ...
            postdata.push(totalobjs);
            postdata.push(changerow);
    ...
    for (var i = 0; i < lofdata * 20; i = i + 20) {
      var arraytosend = postdata.slice(i, i + 20);
            ...
            toast.error(
              "Can't perform this action because of detail type rows"
            );
          }
        } else {
          toast.error("please select corresponding total");
        }
      } else {
        toast.error("please select corresponding header and total");
      }
    } else {
      toast.error("select correct header and total");
    }
  };
  exportdata = async () => {
    await this.setState({
      ...
      if (rowsList.length > 0) {
        ...
        cols.map(v => {
          var row = this.state.orderColumns.filter(
            u => u.ColumnName.toUpperCase() == v.toUpperCase()
          );
          if (row.length > 0) {
            row.map(o => {
              colsguid.push(o);
        ...
        rowsList.map((u, i) => {
          if (i != 0) {
            ...
            r = data.find(
              ii =>
                ii.Columns[0].obj.AmountValue == u[0] ||
                ii.Columns[0].obj.TextValue == u[0]
            );
            var r1 = data1.find(
              ii =>
            ...
            if (r) {
              ...
              gosl.map(iu => {
                let obj = {
                  ...
                  AmountValue: iu.obj.AmountValue,
  ...
  addImportInDatabase = async finalobjects => {
    ...
    //     console.log(this.state.templates, 'allColumnsOfTemplates', '--------------------------------------------');
  };
                      ...
                      {data.Columns.length > 0
                        ...
                        ? data.Columns.map((cd, i) =>
                          ...
                          this.state.orderColumns[i].Hide == false ? (
                            ...
                            this.state.orderColumns[i].ColumnName == "Description" ? (
                                ...
                                {cd.obj.TextValue == " " ||
                                 ...
                                 //   className={
                                     ...
                                     // cd.obj.ColumnGuid === this.state.orderColumns[i].guid ? "text-"+this.state.orderColumns[i].Alignment :''
                                      ...
                                      // this.state.orderColumns[i].Format == 'TEXT' ? 
                                      // data.Type === "Detail" && this.state.orderColumns[i].Type === "Entry" ?
                                      //  'text-primary text-left' : 'text-left' : data.Type === "Detail" && this.state.orderColumns[i].Type === "Entry" ?
                                      //   'text-primary text-right' : "text-right"
