          ...
          if (e.shiftKey) {
            currCell.prev().focus();
              ...
              if(currCell.next().length>0){
                    ...
                    //  console.log (currCell,"hhhhhhhhhhhhhhhh");
                currCell.next().focus();
              }else{
...
if(
(d[ii].Columns[0].obj.TextValue === "" || d[ii].Columns[0].obj.TextValue ===null|| d[ii].Columns[0].obj.TextValue ===" ") &&
(d[ii].Columns[0].obj.AmountValue === "0" || d[ii].Columns[0].obj.AmountValue === "")
){
...
allcolumnsoftemp.find(col => col.Formula && col.Formula.includes(guid))
);
if(columnInFormulaExist === undefined) {
var columnguids = [];
...
toast.error("you can't delete this template because template's column used in formula.");
}
await this.setState({
isLoading: false,
});
...
//            document.activeElement.textContent=this.formatValidation(this.state.editCellValue,"[Green]0%;[Red]-0%")[0].value   
  ...
  makeposition = position => {
    ...
    prev_position =(prev_position.length>8)? prev_position.slice(0, -4)+(parseInt(prev_position.substr(prev_position.length-4)) + 10).toString() : (parseInt(prev_position)+10).toString();
    ...
    if (
      ...
      prev_position =(prev_position.length>8)? (prev_position.slice(0, -4)+(parseInt(prev_position.substr(prev_position.length-4)) - 10).toString()).toString() + "5000":(parseInt(prev_position-10).toString()+"5000");
      if (
        this.state.originalData.find(
          e => e.Position === prev_position.toString()
        ) !== undefined
...
var pivotdataupdate=[];
   let checkedRowsHeaders = [];
    ...
    if (check_row.length > 0) {
      ...
      if (this.state.delete == true) {
          ...
          if(check_row.find(i=>i.Type=="Total"||i.Type=="Header")==undefined){
              //details delete case
 ...
 value.deletes = true;
          ...
          }else if(check_row.find(i=>i.Type=="Total")!==undefined
              ...
              if(check_row.length===1&&check_row[0].Type==="Header"&&check_row[0].ParentGuid!=="root"){
                  //delete chunk
                var array = [];
                var guid =check_row[0].guid;
                  var containdetail=false;
                  var headerindex= this.state.pivotGroupTag.findIndex(h=>h.guid===guid);
                  var totalindex= this.state.pivotGroupTag.findIndex(h=>h.ParentGuid===guid&&h.Type==="Total");
...
if(array.length>0){
    var levelguid=this.state.orderColumns.find(u=>u.ColumnName.toUpperCase()==="LEVEL").guid
    array.map(o=>{
           
    if(o.Type==="Header"){
        
    
       this.state.pivotGroupTag.find(u => u.guid == o.guid).ParentGuid=parentid;
           this.state.finaldata.find(u => u.guid == o.guid).ParentGuid=parentid;
             this.state.originalData.find(u => u.guid == o.guid).ParentGuid=parentid;
   
    }
        var tmprowtag=this.state.pivotGroupTag.find(u => u.guid == o.guid)
       var leveltag= tmprowtag.Columns.find(y=>y.obj.ColumnGuid===levelguid)
        if(parseInt(leveltag.obj.TextValue)!=NaN){
            leveltag.obj.TextValue=parseInt(leveltag.obj.TextValue)-1
        }else if(parseInt(leveltag.obj.AmountValue)!=NaN){
            leveltag.obj.TextValue=parseInt(leveltag.obj.AmountValue)-1
        }
       
        var tmproworg=this.state.originalData.find(u => u.guid == o.guid)
        
        var levelorg= tmproworg.Columns.find(y=>y.ColumnGuid===levelguid)
        ...
        pivotdataupdate.push(this.state.originalData.find(u => u.guid == o.guid));
    })
                    ...
                    e.ID == 1
                    ? this.setState({
                    message_desc: "select only header which have no Detail rows to delete header and it's total",
                    message_heading: "Delete Row",
                    openMessageModal: true
                    })
                    : ""
              ...
              }else if(check_row.find(i=>i.Type=="Header")!==undefined
                   &&check_row.find(i=>i.Type=="Detail")!==undefined){
                          this.state.required_messages.map(e =>
                    e.ID == 1
                    ? this.setState({
    ...
    if (
      ...
      if (
        ...
        // We have to paste complete chunk parallel to extreme child nodes that not having further child nodes
          ...
          cutRows[0].Type === "Header" && (pasteRow.Type === "Header" || pasteRow.Type === "Total")
        ) {
          /**
      ...
      if(selectedrows.length == 1){
        ...
        selectedrows[0].Type === "Total"
               ...
               //insert header and total process   
                  ...
                  if(selectedrows[0].Type === "Header"&&selectedrows[0].ParentGuid!=="root"){
                      //check selected header have childs or not
                  var pasteRowHaveChunks;
            pasteRowHaveChunks = pt.find(
              row => row.ParentGuid === selectedrows[0].guid && row.Type === "Header"
            );
                      if(pasteRowHaveChunks===undefined){
          ...
          "Correct Your Selection of Row(Header/ total) To Insert Header And Total"
                    ...
                    o => o.guid === selectedrows[0].guid
                     ...
                     if(this.state.pivotGroupTag[indxoffst1h-1].Type==="Total"||(this.state.pivotGroupTag[indxoffst1h-1].Type==="Header"&&this.state.pivotGroupTag[indxoffst1h-1].guid===selectedrows[0].ParentGuid)){
            ...
            k => k.ColumnName.toUpperCase() === "LEVEL"
                         ...
                         var finallevel1h=0;
                          var tmp_lvl= this.state.pivotGroupTag[indxoffst1h-1].Columns.find(k=>k.obj.ColumnGuid===levelcol1h.guid)
                     if(this.state.pivotGroupTag[indxoffst1h-1].Type==="Header"&&levelcol1h!==undefined){
                         //increasing level by one
                       
                        if(tmp_lvl.obj.TextValue==" "||tmp_lvl.obj.AmountValue==null){
                    ...
                    var allcols = od.find(j => j.guid === this.state.pivotGroupTag[indxoffst1h - 1].guid);
                    allcols.Columns.map(ioi => {
                    //get column name to get value except empty columns
...
//              
          ...
          delete singleorgrow["AmountValue"];
                ...
                sampledetail.Columns.map(s=>{
                    if(singleorgrow.Columns.find(h=>h.ColumnGuid===s.ColumnGuid)===undefined){
...
*/
var headers = fetched_data.filter(p => p[0] && p[0].toUpperCase() === 'Header'.toUpperCase());
var totals = fetched_data.filter(p => p[0] && p[0].toUpperCase() === 'Total'.toUpperCase());
...
Type:
row[0] && row[0].toUpperCase() === 'Header'.toUpperCase()
? 'Header'
: row[0] && row[0].toUpperCase() === 'Total'.toUpperCase()
? 'Total'
: 'Detail',
};
...
fdata.Columns = fdata.Columns.concat(objtopush);
     ...
     var colsofcurrenttemp = this.state.allcolumnsoftemp.filter(
        s => wtemplate.Columns.find(k => k === s.guid&&s.Type!="System") !== undefined
      );
    var columnsForDB=[];
...
this.setState({finalColumnsForDB: this.state.finalColumnsForDB.concat(columnsForDB)});
});
}
...
//making temp taht have to be new created end
    ...
    temps.map((s, ind) => {
      ...
      var colsofcurrenttemp = this.state.allcolumnsoftemp.filter(
        h => completeobjtmp.Columns.find(k => k === h.guid&&h.Type!=="System") !== undefined
      );
...
color = "red";
(value != "") ? value = "(" + parseFloat(value.toString().substring(1)).toFixed(2) + ")": value = "0";
...
*/
let { originalData, pivotGroupTag, orderColumns } = this.state;
let DMASRule = {
'(': '(',
')': ')',
'/': '/',
...
calculatedTypeColumns = orderColumns.filter(column => column.Type === "Calculation" && (column.Formula !== null && column.Formula !== ""));
...
let column = calculatedTypeColumns.find(column => column.guid === cell.obj.ColumnGuid);
if(column !== undefined && column.Formula !== null && column.Type !== "System") {
/**
...
let expression = formula.split(' ').map(formulaGuid => {
if(isNaN(formulaGuid) || DMASRule[formulaGuid] === undefined) {
// let originalDataCell = originalData[rowIndex].Columns.find(originalDataCell => originalDataCell.ColumnGuid === formulaGuid);
let originalDataCell = originalData.find(
originalDataRow => originalDataRow.guid === pivotGroupTagRow.guid
).Columns.find(originalDataCell => originalDataCell.ColumnGuid === formulaGuid);
if(originalDataCell !== undefined) {
if(
...
await this.setState({ pivotGroupTag: pivotGroupTag });

}
removeSpacesBetweenNumbers = (formula) => {
let newFormula = "";

formula.toString().split(' ').map((item, index) => {
...
newFormula = newFormula.split(' ').filter(item => item !== "").join(' ');
return newFormula;
}
removeCutRows = async () => {
                              ...
                              <>
                                ...
                                {cd.obj.TextValue == " " ||
                                  ...
                                  cd.obj.TextValue == null ? (
                                    ...
                                    <><td className={data.clicked ? "opened" : "closed"} style={{fontSize:this.state.fontsizetemplate ==0? "":this.state.fontsizetemplate, "width": "20px" }} suppressContentEditableWarning={true} onKeyDown={(e) => (e.keyCode == 32) ? (data.carrot) ? this.ongroupclick(e, data.guid) : "" : e.preventDefault()} tabindex={i} > {data.carrot ? (
                                      ...
                                      <td style={{fontSize:this.state.fontsizetemplate ==0? "":this.state.fontsizetemplate, "width": (this.state.orderColumns[i].Width * 6) + "px" , "text-align":cd.obj.ColumnGuid === this.state.orderColumns[i].guid ? this.state.orderColumns[i].Alignment :'text-center'
                                      ...
                                      )} suppressContentEditableWarning={true} onKeyDown={(e)=>this.handleChangeCell(e,this.state.orderColumns[i].Format)}
                                  ...
                                  (<><td className={data.clicked ? "opened" : "closed"} style={{fontSize:this.state.fontsizetemplate ==0? "":this.state.fontsizetemplate, "width": "20px" }} suppressContentEditableWarning={true} onKeyDown={(e) => (e.keyCode == 32) ? (data.carrot) ? this.ongroupclick(e, data.guid) : "" : e.preventDefault()} tabindex={i} > {data.carrot ? (
                                    ...
                                    <td style={{fontSize:this.state.fontsizetemplate ==0? "":this.state.fontsizetemplate, "width": (this.state.orderColumns[i].Width * 6) + "px",
                                    "text-align":cd.obj.ColumnGuid === this.state.orderColumns[i].guid ? this.state.orderColumns[i].Alignment :'text-center'
                                      ...
                                      onKeyDown={(e)=>this.handleChangeCell(e,this.state.orderColumns[i].Format)}
                                  ...
                                  <td style={{fontSize:this.state.fontsizetemplate ==0? "":this.state.fontsizetemplate,color:this.formatValidation( this.state.orderColumns[i].Format == 'TEXT' && cd.obj.AmountValue == 0 && cd.obj.TextValue == null ? "" : 
                                    ...
                                    onKeyDown={(e)=>this.handleChangeCell(e,this.state.orderColumns[i].Format)} contentEditable={((data.Type === "Detail" &&
                                  ...
                                  <td style={{fontSize:this.state.fontsizetemplate ==0? "":this.state.fontsizetemplate,color:this.formatValidation(cd.obj.TextValue,this.state.orderColumns[i].Format )[0].color, "width": (this.state.orderColumns[i].Width * 6) + "px" , "text-align":cd.obj.ColumnGuid === this.state.orderColumns[i].guid ? this.state.orderColumns[i].Alignment :'text-center'}} suppressContentEditableWarning={true}
                                  ...
                                  >
                                    ...
                                    {this.formatValidation(cd.obj.TextValue,this.state.orderColumns[i].Format )[0].value}
                            ...
                            this.state.orderColumns[i].ColumnName.toUpperCase() == "DESCRIPTION" ? (
                              ...
                              <>
                                ...
                                {cd.obj.TextValue == " " ||
                                  ...
                                  cd.obj.TextValue == null ? (<><td className={data.clicked ? "opened" : "closed"} style={{fontSize:this.state.fontsizetemplate ==0? "":this.state.fontsizetemplate, "width": "20px" }} suppressContentEditableWarning={true} onKeyDown={(e) => (e.keyCode == 32) ? (data.carrot) ? this.ongroupclick(e, data.guid) : "" : e.preventDefault()} tabindex={i}>    {data.carrot ? (
                                    ...
                                    <td style={{fontSize:this.state.fontsizetemplate ==0? "":this.state.fontsizetemplate, "width": (this.state.orderColumns[i].Width * 6) + "px" }} onBlur={(e) => this.handleChangeCellOnEnter(
