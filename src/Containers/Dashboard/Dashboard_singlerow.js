...
class Dashboard extends Component {
  ...
  openAddEditModal = () => {
    ...
    if (selectedTemplate && selectedTemplate.guid) {
      this.openModal("openAddEditColumn");
      $(document).ready(function () {
        $(this).find(".column_select_focus").focus();
      });
    } else {
      toast.error("Please Select Template First!");
    }
  ...
  handleChangeCell = async (e, format) => {
    if (this.state.editCellId !== "") {
      ...
      if (e.keyCode === 9) {
        ...
        //shift+tab
        if (e.shiftKey) {
          document.activeElement.textContent = this.formatValidation(
              ...
              if (valstoreplace !== undefined) {
                if (x.Format == "TEXT") {
                  ...
                  if (
                    ...
                    obj[x.guid] =
                      u.Type == "Total"
                        ? parseFloat(valstoreplace.obj.AmountValue).toFixed(2)
                        : parseFloat(valstoreplace.obj.AmountValue);
                  } else {
                    obj[x.guid] =
                      u.Type == "Total"
                        ? parseFloat(valstoreplace.obj.TextValue).toFixed(2)
                        : parseFloat(valstoreplace.obj.TextValue);
                  }
                }
              }
            } else {
              ...
              .map((formulaGuid) => {
                if (isNaN(formulaGuid) || DMASRule[formulaGuid] === undefined) {
                  ...
                  let originalDataCell = originalData
                    .find(
                      (originalDataRow) =>
                        originalDataRow.Position === pivotGroupTagRow.Position
                    )
                    .Columns.find(
                      (originalDataCell) =>
                  ...
                  if (originalDataCell !== undefined) {
                    ...
                    else formulaGuid = originalDataCell.AmountValue;
                  }
                }
                return formulaGuid;
              })
                              ...
                              this.state.orderColumns[
                                ...
                                <td
                                  style={{
                                    ...
                                    width:
                                      this.state.orderColumns[i].Width + "vw",
                                    "text-align":
                                      cd.obj.ColumnGuid ===
                                      this.state.orderColumns[i].guid
