...
class Dashboard extends Component {
      ...
      $('td').keyup(function (e) {
        ...
        if (e.which === 9) {
          ...
          if (e.shiftKey) {
            // currCell.prev().focus();
            let breaker =
              currCell.prev()[0].getAttribute('tagnext') === null ? true : currCell.prev()[0].getAttribute('tagnext') === "false" ? true : false;
            let nexttd = currCell.prev();

            if (breaker) {
              let rowLength = document.getElementsByTagName("tr")[0].childElementCount - 1;
  ...
  componentDidMount = async () => {
    ...
    if (!this.props.isAuthenticated) {
      ...
      if (!guid) {
        ...
        let templateName = this.state.templates.find(template => template.guid === this.state.selectedtempguid).TemplateName;
        this.activityRecord([{
  ...
  getTableBodyHeight = () => {
    if(document.getElementById("table_height")) {
      console.log("getTableBodyHeight ------------- getTableBodyHeight");
      
      let children = Array.from(document.getElementById("table_height").children);
      ...
      let newTableBodyHeight = window.innerHeight - (navHeight + tableHeaderHeight + 14);

      if(childrenHeight < newTableBodyHeight) {
        newTableBodyHeight = childrenHeight;
      }
      document.getElementById("table_height").style.height = `${newTableBodyHeight}px`;
    }
  }
  ...
  getprojects = async () => {
    ...
    await API.post('pivot', '/getProjects', {
      ...
      .then((data) => {
        ...
        let user_type = history.location.state.user_type ? history.location.state.user_type : this.state.user_type;
        console.log("getprojects ----------------------------------------  ", user_type);
...
localStorage.setItem('templateguid', JSON.stringify(guids));
  ...
  getTemplateByGuid = async (guid) => {
    ...
    if (obj !== '' && obj !== null) {
      ...
      await this.setState({
        ...
        fontsizetemplate: this.state.selectedProject.FontSize ? this.state.selectedProject.FontSize : 0,
        ...
        onlyExpenseAcc: this.state.editTemplateModal && this.state.editTemplateModal.ExpenseOnly ?
          this.state.editTemplateModal.ExpenseOnly : false,
      });
    } else {
      await this.updatetemplateinlocal(this.state.templates[0].guid);
      await this.gettemplateanddata(this.state.templates[0].guid);
  ...
  handleSelectChange = async (event) => {
    ...
    let selectedtempguid = JSON.parse(JSON.stringify(this.state.selectedtempguid));
    ...
    this.activityRecord([{
      ...
      "Projectguid": this.state.selectedProject.guid,
      "ColumnName": "",
      "ValueFrom": this.state.templates.find(temp => temp.guid === selectedtempguid).TemplateName,
      "ValueTo": this.state.templates.find(temp => temp.guid === tempguid).TemplateName,
      "Tenantid": localStorage.getItem('tenantguid')
    }]);
    $('#closeRightSliderArrow').click();
  };
  ...
  groupingLevel = async (pivotdata) => {
    ...
    pivotdata = updatedPivotTag.nonHiddenRows;
    var data = pivotdata;
    var columnguid = '';
    var objtmp;
  ...
  handleChangeCheckbox = async (checkBoxName, isChecked) => {
    ...
    if (checkBoxName === 'on_conflict_hide_group_list') {
      let previousValue = JSON.parse(JSON.stringify(this.state.onConflictHideGroupsList));
      await this.setState({
    ...
    if (checkBoxName === 'onlyExpenseAcc') {
      this.changeExpenseOnly(isChecked);
    }
  ...
  changefont = async (select) => {
    ...
    await API.post('pivot', '/updatefields', {
      ...
      .catch(async (err) => {
        await toast.error('Template Font Not Updated');
        await that.setState({
          isLoading: false
        });
      });
  ...
  changeExpenseOnly = (isChecked) => {
    console.log("changeExpenseOnly", isChecked);
      ...
      if (access_token && refresh_token) {
        axios.get('https://cors-anywhere.herokuapp.com/https://api.xero.com/connections', {
          ...
          .catch(async (myBlob) => {
            ...
            refreshCounter++;
            if (refreshCounter < 2) {
              await this.refreshXeroToken(event);
            } else {
...
console.log(this.state.selectedProject.guid,'guids----------------------------------',exist);
    if (exist.length > 0) {
    ...
    if (this.state.editTemplateModal === '') {
      ...
      await API.post('pivot', '/updatefields', {
        ...
        .then(async (data) => {
          ...
          let template = templates.find(template => template.guid === selectedtempguid);
          let previousValues = [];
          let newValues = [];
          let dateTime = new Date().getTime();
          ...
          template.Exclude[0]["HeadingsTotals"] = this.state.check_heading_totals;
          template.Exclude[0]["HiddenColumns"] = this.state.check_hidden_columns;
  ...
  applyingfilter = async () => {
    ...
    if (this.state.check_blank_row) {
      data_filter.map((u) => {
        ...
        if (u.Columns) {
          var levelguid="";
  levelguid= this.state.orderColumns.find(k=>k.ColumnName.toUpperCase() == 'LEVEL').guid
     /*           
      if (vals.ColumnName.toUpperCase() == 'LEVEL') {
        levelguid = vals.guid;
      }*/
          u.Columns.map((i) => {
            ...
            if (!(i.obj.AmountValue == 0 && (i.obj.TextValue == null||i.obj.TextValue ==0))) {
              fla = true;
            }
          }
          });
  ...
  updatetemplatename = async (e) => {
    ...
    let oldTemplateName = JSON.parse(JSON.stringify(
      this.state.templates.find(temp => temp.guid === this.state.selectedtempguid).TemplateName
    ));
  ...
  xeroTrackingCategories = async () => {
    ...
    if (this.state.currentproject.Connection !== "CSV") {
      ...
      if (accessToken && refresh_token && xeroTenantId && this.state.currentproject.guid === localStorage.getItem('selectedProjectId')) {
        ...
        //        e.ID == 17 ?
        //        this.setState({
        //          message_desc: 'Tracking catagories not fetched please connect xero account to proceed this process.',
        //          message_heading: "Xero",
        //          openMessageModal: true,
        //        }) :
        //        ''
  ...
  handleClickOnCell1 = async (id, data, cellValue) => {
    ...
    if (this.state.indexfocus !== currenttabindex) {
      ...
      await this.setState({
          editCellId: id,
          editCellValue: document.activeElement.textContent
        },
        () => {}
      );
    }
      ...
      if (this.state.editCellId !== '') {
        ...
        if (e.keyCode === 27) {
          ...
          if (
            ...
            //  console.log('22222222222--------------------------2--------------: ', e.target.textContent);
          }
        }
        //console.log("hhhhhhhhhhhhhhhhhhhhhhhhh",this.state.editCellValue)
        ...
        if (
          ...
          finalval = await this.formatValidation(this.state.editCellValue, format)[0].value;
        }
  ...
  handleChangeCellOnEnter = async (
    ...
    console.log("cellObj", cellObj, "editCellValue", this.state.editCellValue);
    await this.removeCheckedRows();
    await this.redRowHandler();
...
//        }
        ...
        let dateTime = new Date().getTime();
        let column = this.state.allcolumnsoftemp.find(col => col.ColumnName.toUpperCase() === "ACCOUNT");

        let cellColumn = this.state.allcolumnsoftemp.find(col => col.guid === cellObj.obj.ColumnGuid);
        let account = undefined;
        if (column) {
          let accountCell = data.Columns.find(cell => cell.obj.ColumnGuid === column.guid);
          if (accountCell) {
            if (accountCell.obj.TextValue && accountCell.obj.TextValue !== "" && accountCell.obj.TextValue !== " ") {
        ...
        let cell_column = this.state.allcolumnsoftemp.find(column => column.guid === FROM.COLUMNGUID);
...
if(!this.state.check_blank_row){
  ...
  onDeleteRow = async () => {
    ...
    let descriptionColumnGuid = this.state.allcolumnsoftemp.find(col => col.ColumnName.toUpperCase() === "DESCRIPTION").guid;
    ...
    this.state.finaldata.map((e, index) => {
      if (e.checkbox == true ) {
        ...
        let descriptionCell = e.Columns.find(cell => cell.obj.ColumnGuid === descriptionColumnGuid);
    ...
    check_row.map((checkedRow) => {
      ...
      periodcolumn.map(k => {
        ...
        if (
          ...
          (
            originalCell && originalCell.TextValue !== null &&
            originalCell && originalCell.TextValue != "0"
          )
        ) {
          emptyperiodcell = false;
        }
    ...
    if (emptyperiodcell === true) {
      ...
      if (check_row.length > 0) {
        ...
        undoPivotRows.push(this.state.pivotGroupTag.find(row => local_guid === row.guid));
        undoOriginalRows.push(this.state.originalData.find(row => local_guid === row.guid));
        });
        undo.push({
        "DELETEDROW": {
  ...
  pasteRow = async () => {
    ...
    let { undo } = this.state;
    let undoPasteRows = [];
    let levelcol = this.state.allcolumnsoftemp.find((k) => k.ColumnName.toUpperCase() === 'LEVEL');
        ...
        if (cutRows[0].Type === 'Detail') {
          cutRows.map(async (row) => {
            ...
            if (pasteRow.Type === 'Header') {
              ...
              let cell = row.Columns.find(cell => cell.obj.ColumnGuid === levelcol.guid);
              undoPasteRows.push({
                "GUID": row.guid,
  ...
  goToActivity = (e) => {
    ...
    history.push('/activity', {
      ...
      user_type: history.location.state.user_type ? history.location.state.user_type : this.state.user_type,
      ...
      if (
        ...
        if (
          ...
          if (
            ...
            this.setState({
              message_desc: "Header & Total cannot be inserted on checked rows. Please refer to help menu for more information.",
          ...
          if (levelcol !== undefined) {
            for (var i = newheaderindx + 1; i < newtotalindx; i++) {
              ...
              if (pt[i].ParenGuid === selectedrows[1].guid) {
                ...
                od.find(k => k.guid === pt[i].guid).ParentGuid = parentidtoreplace;
              }
          ...
          if (
            ...
            if (i !== undefined) {
              if (x.Type == 'System') {
                ...
                let cell = i;
                if (x.Type.toUpperCase() == 'CALCULATION') {
  ...
  formatValidation = (value, format) => {
    ...
    if (value === '') {
      ...
      if (formats[0].val === format) {
        ...
        if (value >= 0) {
          value = value.toString();
          value != '' ? (value = (parseFloat(value * 100).toFixed(2)) + '%') : (value = '0');
        } else {
          color = 'red';
          value = value.toString();
          value != '' ? (value = (parseFloat(value * 100).toFixed(2)) + '%') : (value = '0');
        ...
        pivotGroupTagRow.Columns.map(async (cell, cellIndex) => {
          ...
          if (
            ...
            formula
              ...
              .map(async (formulaGuid) => {
                ...
                if (isNaN(formulaGuid) && DMASRule[formulaGuid] === undefined) {
                  ...
                  if (column.Formula) {
                    let originalRow = originalData.find(k => k.guid === pivotGroupTagRow.guid);
                    // //denugger;;
                    value = this.formulaCallback(originalRow.guid, column.Formula);
                    // console.log(
            ...
            if (periodCount === 0 && column.DoNotCalculate === true) {
              ...
              cell.obj.TextValue = isNaN(tmp) || tmp === -Infinity || tmp === Infinity ? 0 : tmp;
            }
  ...
  handleUndo = async () => {
    ...
    let { undo, originalData, pivotGroupTag, pivotGroupTagHiddenRows, finaldata } = this.state;
    if (undo.length > 0) {
      ...
      switch(operationName) {
        ...
        case "DELETEDROW":
          ...
          await this.undoDelete(lastItem["DELETEDPIVOTROW"], lastItem["DELETEDORIGINALROW"]);
        ...
        case "APPLYGROUPS":
          let groupColumnGuid = this.state.allcolumnsoftemp.find(column => column.ColumnName.toUpperCase() === "GROUPS").guid;
          let postData = [];
          await this.getGroups();
          ...
          await lastItem["APPLYGROUPS"].map(row => {
            let currentTagRow = pivotGroupTag.find(pivotRow => pivotRow.guid === row.guid);
            currentTagRow = currentTagRow ? currentTagRow : pivotGroupTagHiddenRows.find(pivotRow => pivotRow.guid === row.guid);
                
            var g = pivotGroupTag.findIndex((v) => row.guid == v.guid);
            let pi = [];
            pi.push(g);
            var ph = pivotGroupTagHiddenRows.findIndex((v) => row.guid == v.guid);
            let phr = [];
            phr.push(ph);
            ...
            row.Columns.map(async (cell, index) => {
              if(cell.obj.ColumnGuid === groupColumnGuid) {
                ...
                await this.undoApplyGroup(obj, unobj, originalData, pivotGroupTag, pivotGroupTagHiddenRows, finaldata, pi, phr, od, fi, hiddenGroupNames, removeRowsIndexes);
              }
            });
          ...
          pivotGroupTagHiddenRows = pivotGroupTagHiddenRows.filter((row, index) => !removeRowsIndexes.includes(index));
  ...
  undoDelete = async (pivotRows, originalRows) => {
    ...
    let postData = [];
    let levelcol = this.state.allcolumnsoftemp.find(column => column.ColumnName.toUpperCase() === "LEVEL");
    
    pivotRows.map(row => {
      if(row.Type.toUpperCase() === "HEADER" || row.Type.toUpperCase() === "TOTAL") {
        rowsHasHeader = true;
      }
    });
    ...
    if(rowsHasHeader) {
      ...
      let headerCheck = pivotGroupTag.find(row => row.guid === pivotRows[0].ParentGuid);
  ...
  undoInsertColumn = async (guid) => {
    await this.isLoading(true);
    let { allcolumnsoftemp, selectedtempguid, templates, originalData } = this.state;
    let columnData = allcolumnsoftemp.find(column => column.guid === guid);
    let allColumns = allcolumnsoftemp;

    if (columnData && columnData.guid) {

      let findColumnInFormula = allColumns.find(column => column.Formula !== null &&
        column.Formula.toString().includes(columnData.guid) &&
        columnData.guid !== column.guid
      );
      let findColumn = allColumns.find(column => column.SelectedColumnGuid === columnData.guid);

      if (findColumnInFormula) {
        this.setState({
          message_desc: 'There is a formula referenced to this template/column so it can’t be deleted. Remove from formula and try again.',
        ...
        this.setState({
          message_desc: 'This column is referenced with other column. Try to remove this reference first.',
          message_heading: 'Delete',
          openMessageModal: true
        });
        return;
        ...
        if (columnData && columnData.guid) {
          ...
          if (deleteColumn) {
            let selectedTemp = templates.find(temp => temp.guid === selectedTempGuid);
          ...
          newArray.map(pod => {
            if (pod.Type !== "Blank") {
              pod.Columns = pod.Columns.filter(c => c.ColumnGuid !== columnData.guid);
            }
          });
  ...
  undoCellChange = async (WORKTABLECHANGE) => {
    if(WORKTABLECHANGE) {
      ...
      let originalRow = originalData.find(row => row.guid === WORKTABLECHANGE.ROWGUID);
      let originalCell = originalRow.Columns.find(cell => cell.ColumnGuid === WORKTABLECHANGE.FROM.COLUMNGUID);
      let pivotRow = pivotGroupTag.find(row => row.guid === WORKTABLECHANGE.ROWGUID);
      let pivotCell = pivotRow.Columns.find(cell => cell.obj.ColumnGuid === WORKTABLECHANGE.FROM.COLUMNGUID);
  ...
  undoApplyGroup = async (obj, unobj, originalData, pivotGroupTag, pivotGroupTagHiddenRows, finaldata, pi, phr, od, fi, hiddenGroupNames, removeRowsIndexes) => {
    var x = this.state.orderColumns.find(
      (u) => u.ColumnName.toUpperCase() == 'GROUPS'
    );
    var groupguid = x.guid;
    ...
    od.map((pushind) => {
      if(originaldata[pushind]) {        
        ...
        if (tmp_obj.TextValue === null || tmp_obj.TextValue === ' ') {
          var psrval = tmp_obj.AmountValue !== null && tmp_obj.AmountValue !== '' ?
            tmp_obj.AmountValue.split(',') : [];
          Uncheckary.map((o) => {
            if (psrval.indexOf(o) > -1) {
              delete psrval[psrval.indexOf(o)];
  ...
  undoHeaderTotal = async (data) => {
    await this.isLoading(true);
    let { originalData, pivotGroupTag, finaldata, allcolumnsoftemp } = this.state;
    ...
    let chunkRows = [];
    let postData = [];
    let levelcol = allcolumnsoftemp.find(column => column.ColumnName.toUpperCase() === "LEVEL");
    ...
    if(chunkRows.length > 0) {
      ...
      originalData = originalData.filter(row => row.guid !== data["HEADERGUID"] && row.guid !== data["TOTALGUID"]);
      pivotGroupTag = pivotGroupTag.filter(row => row.guid !== data["HEADERGUID"] && row.guid !== data["TOTALGUID"]);
      finaldata = finaldata.filter(row => row.guid !== data["HEADERGUID"] && row.guid !== data["TOTALGUID"]);
      ...
      for (var i = 0; i < lofrows * 20; i = i + 20) {
        ...
        await API.post('pivot', '/deletepivotdata', {
          ...
          .catch((data) => toast.error("Rows failed to delete."))
      }
  ...
  undoPasteRows = async (pivotRowData) => {
    ...
    pivotRowData.map(item => {
      ...
      let originalCell = originalRow.Columns.find(cell => cell.ColumnGuid === item.LEVELCOLUMNGUID);
      
      let pivotRow = pivotGroupTag.find(row => row.guid === item.GUID);
      let pivotCell = pivotRow.Columns.find(cell => cell.obj.ColumnGuid === item.LEVELCOLUMNGUID).obj;
  ...
  handleRunReport = async () => {
    ...
    if (this.state.reportdata) {
      ...
      this.setState({
        ...
        message_desc: "There are no report defaults for this template. Run report from slide out menu and add defaults.",
        openMessageModal: true,
      });
    }
  ...
  refreshXeroToken = async (optional = {}) => {
    let refresh_token = JSON.parse(localStorage.getItem('refresh_token'));
    if (refresh_token) {
      ...
      await axios({
        ...
        .then(async (response) => {
          ...
          switch (refreshFunction) {
            ...
            case "switchProject":
              refreshFunction = undefined;
              refreshCounter = 0;
              await this.switchProject(optional);
              break;
            case "Refreshapicall":
              refreshFunction = undefined;
              refreshCounter = 0;
              await this.Refreshapicall();
              break;
            default:
              break;
          }
        ...
        .catch(async (myBlob) => {
          if (refreshFunction === "switchProject") {
            await this.switchProject(optional);
          }
          this.state.required_messages.map((e) =>
            e.ID == 17 ?
  ...
  Refreshapicall = async () => {
    ...
    await axios({
      ...
      .catch(async (myBlob) => {
        refreshFunction = "Refreshapicall";
        refreshCounter++;
        if (refreshCounter < 2) {
          await this.refreshXeroToken();
        } else {
  ...
  handleRefresh = async () => {
    ...
    if (selectedProject.Connection === "XERO") {
      ...
      let refresh_token = JSON.parse(localStorage.getItem('refresh_token'));
      let xeroTenantId = localStorage.getItem('xeroTenantId');
      if (accessToken && refresh_token && xeroTenantId && selectedProject.guid === localStorage.getItem('selectedProjectId')) {
  ...
  handlingXeroAccounts = async () => {
    ...
    accounts.map((account) => {
      if(this.state.onlyExpenseAcc===false||account.Class==="EXPENSE"){
        ...
        originalData.find((row, index) => {
          ...
          if (index === pivotGroupTag.length - 1 && !isAccountExist) {
            ...
            let sampleOriginalRowColumns = sampleOriginalRow.Columns.map((cell) => {
              ...
              return {
                AmountValue: "0",
                TextValue: cell.ColumnGuid === accountColumnGuid ?
                  code :
          ...
          .then(async (response) => {
            ...
            if (response.data.Reports.length > 0) {
              ...
              sections.map(sec => {
                ...
                finalrows.map(s => {
                  that.replace_valueinDI(data, s, guid, TrackingCodeID, cellunassign)
                });
              });
  ...
  replace_valueinDI = async (data, row, guid, TrackingCodeID,
    cellunassign) => {
    let {
      pivotGroupTag,
      originalData,
        ...
        pivotGroupTag.map((pivotRow) => {
          if (pivotRow.Type !== "Header" && pivotRow.Type !== "Total") {
            ...
            let selectedColumnCell = pivotRow.Columns.find(
              (cell) => cell.obj.ColumnGuid === previousColumn.SelectedColumnGuid
            );
            let selectedColumn = allcolumnsoftemp.find(column => column.guid === previousColumn.SelectedColumnGuid);
            let originalRow = originalData.find(row => row.guid === pivotRow.guid);
            let originalCell = originalRow.Columns.find(cell => cell.ColumnGuid === previousColumn.guid);
            let value = undefined;
            ...
            if (selectedColumnCell) {
              ...
              if (selectedColumn.Formula) {
                ...
                value = this.formulaCallback(originalRow.guid, selectedColumn.Formula);
                console.log('formulaValue -------xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx:    ', value);

              } else {

                value = selectedColumnCell.obj.TextValue;
              ...
              if (originalRow && selectedColumn.Formula) {
                ...
                originalCell = originalRow.Columns.find(cell => cell.ColumnGuid === selectedColumn.guid);
                value = originalCell.TextValue;
            ...
            if (previousPeriodCell) {
              previousPeriodCell.obj.TextValue = value;
              previousPeriodCell.obj.AmountValue = "0";
            }

            originalCell.TextValue = value;
  ...
  generatereport = async (temp = "") => {
    await this.setState({
      isLoading: true
    })
    this.removeCutRows();
    var finalreport = {};
    var templateforreport = {};
    finalreport["Columns"] = this.state.orderColumns;
    templateforreport['TenantGuid'] = this.state.editTemplateModal.TenantGuid;
    templateforreport['Exclude'] = this.state.editTemplateModal.Exclude;
    templateforreport['guid'] = this.state.editTemplateModal.guid;
    templateforreport['BusinessUnitGuid'] = this.state.editTemplateModal.BusinessUnitGuid;
    templateforreport['FontSize'] = this.state.editTemplateModal.FontSize;
    templateforreport['ColumnsList'] = this.state.editTemplateModal.Columns;
    templateforreport['TemplateName'] = this.state.editTemplateModal.TemplateName;
    finalreport["Template"] = templateforreport;
    var data = [];
    this.state.pivotGroupTag.map(h => {
      var f_row = {
        "TenantGuid": h.TenantGuid,
        "guid": h.guid,
        "BusinessUnitGuid": h.BusinessUnitGuid,
        "Cells": [],
        "Position": h.Position,
        "Type": h.Type,
        "ParentGuid": h.ParentGuid,
        "clicked": h.clicked,
        "AmountValue": h.AmountValue,
        "checkzero": h.checkzero,
        "carrot": h.carrot,
        "checkbox": h.checkbox,
      };
      ...
      h.Columns.map(f => {
        f_row.Cells.push({
          AmountValue: f.obj.AmountValue,
          ColumnGuid: f.obj.ColumnGuid,
          TextValue: f.obj.TextValue
        })
    ...
    var path = window.location.protocol + '//' + window.location.host + '/reportpdf?file=' + filename;
  ...
  xeroConnection = async () => {
    ...
    if (selectedProject.Connection === "XERO") {
      ...
      localStorage.setItem("selectedProjectId", selectedProject.guid);
          ...
          >
            ...
            <ul className="nav sidebar-nav">
              ...
              <li>
                ...
                <Link to={{pathname: "/projects", state: { from: this.props.location.pathname }}}>Projects</Link>
          ...
          >
            ...
            <div className="cus-code">
              ...
              <div className="px-3">
                ...
                {
                  this.state.currentproject && this.state.currentproject.Connection === "XERO" ? 
                    <div
...

              {
                   ...
                   <>   <div
                        ...
                        <div className="clearfix"></div>
                        <br/>
                        <p className="tre-group capitalize">{selectedProject && selectedProject.XeroDB ? selectedProject.XeroDB : ""}</p>
                             

                      </div>
                  <div className="r-c-style expense-accounts">
                    <p>
                      <label className="dash_container dash_remember">
  ...
  <span className="dash_checkmark"></span>
        ...
        <div className="sec-table table-responsive  ">
          ...
          {this.state.orderColumns.length > 0 ? (
            ...
            <thead className="he">
                    ...
                    value.Hide == false ? (
                      ...
                      <>
                        ...
                        {value.ColumnName.toUpperCase() == "DESCRIPTION" ? (
                          ...
                          <><th onClick={this.onAllNodes} style={{fontSize:this.state.fontsizetemplate ==0? "":this.state.fontsizetemplate, "width": "20px", "text-align": "center" }}>  
              ...
              <tbody id="table_height" className="table-scrollbar" style={{height: "auto"}}>
                              ...
                              <>
                                ...
                                {cd.obj.TextValue == " " ||
                                  ...
                                  cd.obj.TextValue == null ? (
                                    ...
                                    <>
                                    <td tagNext={"true"} className={data.clicked ? "opened" : "closed"} style={{fontSize:this.state.fontsizetemplate ==0? "":this.state.fontsizetemplate, "width": "20px" }} suppressContentEditableWarning={true} onKeyDown={(e) => (e.keyCode == 32) ? (data.carrot) ? this.ongroupclick(e, data.guid) : "" : e.preventDefault()} tabindex={i} 
        ...
        <ReportsModal
          ...
          current_template={this.state.editTemplateModal.guid ?this.state.temp_report!==""?this.state.temp_report: this.state.editTemplateModal.guid : ""}
